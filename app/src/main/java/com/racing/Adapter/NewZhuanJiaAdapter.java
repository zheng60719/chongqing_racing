...
public class NewZhuanJiaAdapter extends CommonRecyclerAdapter<NewZhuanJiaFenxi> {
    private Context mContext;
    private List<NewZhuanJiaFenxi> dataList;
    private ZhuanJiaOnClick zhuanJiaOnClick;

    public NewZhuanJiaAdapter(Context mContext, List<NewZhuanJiaFenxi> dataList, int resId) {
    ...
    public void setData(RecyclerViewHolder holder, final int position) {
        NewZhuanJiaFenxi fenXi = getItem(position);
        ...
        try {
            ...
            Glide.with(mContext).load(fenXi.getList_jibie().get(0).getLogo()).placeholder(new ColorDrawable(mContext.getResources().getColor(R.color.color_content_bg))).//加载中显示的图片
